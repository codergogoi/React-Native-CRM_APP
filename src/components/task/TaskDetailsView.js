...
class TaskDetailsView extends Component {
  ...
  onSelect = data => {
	...
	taskStatusView = (task) => {
		...
		switch(task.status){
			case 0: // waiting for Accept
				...
				// 		<TouchableOpacity onPress={this.onTapReject} style={[styles.butonView, { width: 180,}]}>
				// 				<Image style={styles.menuIcon} source={RejectIcon} />
				// 				<Text style={styles.buttonText}>Later Followup </Text>
				// 		</TouchableOpacity>
				// 		<TouchableOpacity onPress={this.onTapAccept} style={styles.butonView}>
				// 				<Image style={styles.menuIcon} source={AcceptIcon} />
	...
	render() {
		...
		PRINT(`Image URL Received ${ JSON.stringify(this.props.image_uri)}`);
		...
		if(isAccepted === true){
			Alert.alert(
				'Task Accepted!',
				'You have a new task to accomplish!',
				[
				  {text: 'OK', onPress: () => {
						this.props.navigation.goBack();
						this.props.DismissAlert('')
...
export default connect(mapStateToProps, { AcceptTask, DismissAlert, CompleteTask, UpdateTaskStatus, UpdateImageURI  })(TaskDetailsView);
